...
var flow_path = 'D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag';
...
casper.then(function() {{techo('load ../.txt/report_body.txt to email_body');
var fs = require('fs'); email_body = '';
if (fs.exists('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/../.txt/report_body.txt'))
email_body = fs.read('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/../.txt/report_body.txt').trim();
else this.echo('ERROR - cannot find file ../.txt/report_body.txt').exit();}});
...
if (download_path.indexOf(':')>0) download_path = download_path.replace(/\//g,'\\').replace(/\\/g,'\\');
chrome_step('Page.setDownloadBehavior',{behavior: 'allow', downloadPath: download_path});
chrome_step('Page.navigate',{url: 'https://mail.163.com/'}); sleep(1000);
techo('https://mail.163.com/' + ' - ' + chrome.getTitle());});
...
{sikuli_step('snap page.png to D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find [enter] on screen').exit(); this.wait(0);}}});

// 登录邮箱
// 切换输入法（可能会不同）
casper.then(function() {{techo('keyboard [ctrl][space]'); var fs = require('fs');
if (!sikuli_step('keyboard [ctrl][space]')) if (!fs.exists('[ctrl][space]') && !false)
{this.echo('ERROR - cannot find image file [ctrl][space]').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find [ctrl][space] on screen').exit(); this.wait(0);}}});

// keyboard '+email_username+'
casper.then(function() {{techo('keyboard [tab]'); var fs = require('fs');
if (!sikuli_step('keyboard [tab]')) if (!fs.exists('[tab]') && !false)
{this.echo('ERROR - cannot find image file [tab]').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find [tab] on screen').exit(); this.wait(0);}}});

casper.then(function() {{techo('keyboard '+email_password+''); var fs = require('fs');
if (!sikuli_step('keyboard '+email_password+'')) if (!fs.exists(''+email_password+'') && !false)
{this.echo('ERROR - cannot find image file '+email_password+'').exit(); this.wait(0);} else
{sikuli_step('snap page.png to D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find '+email_password+' on screen').exit(); this.wait(0);}}});
...
// 点击写信
casper.then(function() {{techo('click //*[text()="写 信"]');
casper.waitFor(function check() {return check_tx('//*[text()="写 信"]');},
function then() {chrome.click(tx('//*[text()="写 信"]'));},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find //*[text()="写 信"]').exit(); this.wait(0);});}});

// 输入收件人
casper.then(function() {{techo('type //*[@type="text" and @role="combobox"] as '+email_to+'');
casper.waitFor(function check() {return check_tx('//*[@type="text" and @role="combobox"]');},
function then() {chrome.sendKeys(tx('//*[@type="text" and @role="combobox"]'),''+email_to+'');},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find //*[@type="text" and @role="combobox"]').exit(); this.wait(0);});}});

// 输入主题
casper.then(function() {{techo('type //div[@aria-label="邮件主题输入框，请输入邮件主题"]//*[@class="nui-ipt-input" and @type="text"] as '+email_subject+'');
casper.waitFor(function check() {return check_tx('//div[@aria-label="邮件主题输入框，请输入邮件主题"]//*[@class="nui-ipt-input" and @type="text"]');},
function then() {chrome.sendKeys(tx('//div[@aria-label="邮件主题输入框，请输入邮件主题"]//*[@class="nui-ipt-input" and @type="text"]'),''+email_subject+'');},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find //div[@aria-label="邮件主题输入框，请输入邮件主题"]//*[@class="nui-ipt-input" and @type="text"]').exit(); this.wait(0);});}});

// 设置正文的iframe的name属性，以便于后续操作
casper.then(function() {dom_result = ''; dom_result = chrome.evaluate(function(dom_json) {document.getElementsByClassName("APP-editor-iframe")[0].setAttribute("name","email_body_iframe")}, dom_json);});

// 上传附件
casper.then(function() {{techo('upload .O0 as '+email_attachment+'');
casper.waitFor(function check() {return check_tx('.O0');},
function then() {chrome.upload(tx('.O0'),''+abs_file(''+email_attachment+'')+'');},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find .O0').exit(); this.wait(0);});}});

casper.then(function() {{techo('upload .O0 as '+jpg_attachment_1+'');
casper.waitFor(function check() {return check_tx('.O0');},
function then() {chrome.upload(tx('.O0'),''+abs_file(''+jpg_attachment_1+'')+'');},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find .O0').exit(); this.wait(0);});}});

casper.then(function() {{techo('upload .O0 as '+jpg_attachment_2+'');
casper.waitFor(function check() {return check_tx('.O0');},
function then() {chrome.upload(tx('.O0'),''+abs_file(''+jpg_attachment_2+'')+'');},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find .O0').exit(); this.wait(0);});}});

casper.then(function() {{techo('upload .O0 as '+jpg_attachment_3+'');
casper.waitFor(function check() {return check_tx('.O0');},
function then() {chrome.upload(tx('.O0'),''+abs_file(''+jpg_attachment_3+'')+'');},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find .O0').exit(); this.wait(0);});}});

// 输入正文
casper.then(function() {techo('frame email_body_iframe');});
chrome.withFrame('email_body_iframe', function() {
{ // start of code block

{

casper.then(function() {{techo('type //*[@class="nui-scroll"] as '+email_body+'');
casper.waitFor(function check() {return check_tx('//*[@class="nui-scroll"]');},
function then() {chrome.sendKeys(tx('//*[@class="nui-scroll"]'),''+email_body+'');},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find //*[@class="nui-scroll"]').exit(); this.wait(0);});}});
...
// 点击发送
casper.then(function() {{techo('click //*[text()="发送"]');
casper.waitFor(function check() {return check_tx('//*[text()="发送"]');},
function then() {chrome.click(tx('//*[text()="发送"]'));},
function timeout() {chrome.capture('D:/Horizon/Projects/Ongoing/rpa/flows/work/.tag/report_email_sending_error.png');
this.echo('ERROR - cannot find //*[text()="发送"]').exit(); this.wait(0);});}});

casper.then(function() {techo('wait 5s');});
